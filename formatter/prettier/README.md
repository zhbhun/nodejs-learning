* [官网](https://prettier.io/)
* [仓库](https://github.com/prettier/prettier)
* [文档](https://prettier.io/docs/en/)
* [Playground](https://prettier.io/playground/)

# 用法

* [编辑器](http://prettier.io/docs/en/editors.html)
* [版本提交钩子](https://prettier.io/docs/en/precommit.html)
* [持续集成](https://prettier.io/docs/en/cli.html#list-different)

# 集成

* https://github.com/prettier/prettier-eslint
* https://github.com/hugomrdias/prettier-stylelint
* https://github.com/prettier/prettier-vscode

# 教程

* [用 ESLint 和 Prettier 写出高质量代码](https://egoist.moe/2017/12/11/write-better-code-with-eslint-and-prettier/)
* [JavaScript Code Formatting With Prettier by Christopher Chedeau](https://www.slideshare.net/ReactLondon2017/javascript-code-formatting-with-prettier-by-christopher-chedeau)
* [Achieving Code Consistency](https://survivejs.com/maintenance/code-quality/code-formatting/)
* [Prettier | 一个前端的代码格式化工具](https://zhuanlan.zhihu.com/p/28195771)
* [我为什么推荐 Prettier 来统一代码风格](https://blog.fundebug.com/2017/10/23/format-code-use-Prettier/)
* [笔记, TypeScript 配置 Prettier](https://segmentfault.com/a/1190000011122443)
* [Prettier + ESLint + Facebook Code Quality: The Auto-magical React Styling Tutorial](https://medium.com/@eliotjunior/prettier-eslint-facebook-code-quality-the-auto-magical-react-styling-tutorial-19481acb10dd)
* [What's the difference between prettier-eslint, eslint-plugin-prettier and eslint-config-prettier?
  Ask Question](https://stackoverflow.com/questions/44690308/whats-the-difference-between-prettier-eslint-eslint-plugin-prettier-and-eslint)
* [Your last ESLint config](https://medium.com/@netczuk/your-last-eslint-config-9e35bace2f99)
* [Configure ESLint, Prettier, and Flow in VS Code for React Development](https://hackernoon.com/configure-eslint-prettier-and-flow-in-vs-code-for-react-development-c9d95db07213)
* [Your last ESLint config](https://medium.com/@netczuk/your-last-eslint-config-9e35bace2f99)
